/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   sha512.c                                           :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: bbrassar <bbrassar@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/05/30 11:47:38 by bbrassar          #+#    #+#             */
/*   Updated: 2025/05/30 12:22:39 by bbrassar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "sha2.h"

static inline uint64_t rrot64(uint64_t n, int r)
{
	return (n >> r) | (n << (64 - r));
}

/* clang-format off */

static uint64_t const K[80] = {
	0x428a2f98d728ae22, 0x7137449123ef65cd, 0xb5c0fbcfec4d3b2f,
	0xe9b5dba58189dbbc, 0x3956c25bf348b538, 0x59f111f1b605d019,
	0x923f82a4af194f9b, 0xab1c5ed5da6d8118, 0xd807aa98a3030242,
	0x12835b0145706fbe, 0x243185be4ee4b28c, 0x550c7dc3d5ffb4e2,
	0x72be5d74f27b896f, 0x80deb1fe3b1696b1, 0x9bdc06a725c71235,
	0xc19bf174cf692694, 0xe49b69c19ef14ad2, 0xefbe4786384f25e3,
	0x0fc19dc68b8cd5b5, 0x240ca1cc77ac9c65, 0x2de92c6f592b0275,
	0x4a7484aa6ea6e483, 0x5cb0a9dcbd41fbd4, 0x76f988da831153b5,
	0x983e5152ee66dfab, 0xa831c66d2db43210, 0xb00327c898fb213f,
	0xbf597fc7beef0ee4, 0xc6e00bf33da88fc2, 0xd5a79147930aa725,
	0x06ca6351e003826f, 0x142929670a0e6e70, 0x27b70a8546d22ffc,
	0x2e1b21385c26c926, 0x4d2c6dfc5ac42aed, 0x53380d139d95b3df,
	0x650a73548baf63de, 0x766a0abb3c77b2a8, 0x81c2c92e47edaee6,
	0x92722c851482353b, 0xa2bfe8a14cf10364, 0xa81a664bbc423001,
	0xc24b8b70d0f89791, 0xc76c51a30654be30, 0xd192e819d6ef5218,
	0xd69906245565a910, 0xf40e35855771202a, 0x106aa07032bbd1b8,
	0x19a4c116b8d2d0c8, 0x1e376c085141ab53, 0x2748774cdf8eeb99,
	0x34b0bcb5e19b48a8, 0x391c0cb3c5c95a63, 0x4ed8aa4ae3418acb,
	0x5b9cca4f7763e373, 0x682e6ff3d6b2b8a3, 0x748f82ee5defb2fc,
	0x78a5636f43172f60, 0x84c87814a1f0ab72, 0x8cc702081a6439ec,
	0x90befffa23631e28, 0xa4506cebde82bde9, 0xbef9a3f7b2c67915,
	0xc67178f2e372532b, 0xca273eceea26619c, 0xd186b8c721c0c207,
	0xeada7dd6cde0eb1e, 0xf57d4f7fee6ed178, 0x06f067aa72176fba,
	0x0a637dc5a2c898a6, 0x113f9804bef90dae, 0x1b710b35131c471b,
	0x28db77f523047d84, 0x32caab7b40c72493, 0x3c9ebe0a15c9bebc,
	0x431d67c49c100d4c, 0x4cc5d4becb3e42b6, 0x597f299cfc657e2a,
	0x5fcb6fab3ad6faec, 0x6c44198c4a475817,
};

/* clang-format on */

void sha512_init(struct sha512_context *ctx)
{
	ctx->a = 0x6a09e667f3bcc908;
	ctx->b = 0xbb67ae8584caa73b;
	ctx->c = 0x3c6ef372fe94f82b;
	ctx->d = 0xa54ff53a5f1d36f1;
	ctx->e = 0x510e527fade682d1;
	ctx->f = 0x9b05688c2b3e6c1f;
	ctx->g = 0x1f83d9abfb41bd6b;
	ctx->h = 0x5be0cd19137e2179;
	ctx->length[0] = 0;
	ctx->length[1] = 0;
}

void sha384_init(struct sha512_context *ctx)
{
	ctx->a = 0xcbbb9d5dc1059ed8;
	ctx->b = 0x629a292a367cd507;
	ctx->c = 0x9159015a3070dd17;
	ctx->d = 0x152fecd8f70e5939;
	ctx->e = 0x67332667ffc00b31;
	ctx->f = 0x8eb44a8768581511;
	ctx->g = 0xdb0c2e0d64f98fa7;
	ctx->h = 0x47b5481dbefa4fa4;
	ctx->length[0] = 0;
	ctx->length[1] = 0;
}

static void sha512_compress(struct sha512_context *ctx)
{
	uint64_t a = ctx->a;
	uint64_t b = ctx->b;
	uint64_t c = ctx->c;
	uint64_t d = ctx->d;
	uint64_t e = ctx->e;
	uint64_t f = ctx->f;
	uint64_t g = ctx->g;
	uint64_t h = ctx->h;
	uint64_t w[80];

	for (int i = 0; i < 16; i += 1) {
		w[i] = (((uint64_t)ctx->buffer[i * 8 + 0]) << 56) |
		       (((uint64_t)ctx->buffer[i * 8 + 1]) << 48) |
		       (((uint64_t)ctx->buffer[i * 8 + 2]) << 40) |
		       (((uint64_t)ctx->buffer[i * 8 + 3]) << 32) |
		       (((uint64_t)ctx->buffer[i * 8 + 4]) << 24) |
		       (((uint64_t)ctx->buffer[i * 8 + 5]) << 16) |
		       (((uint64_t)ctx->buffer[i * 8 + 6]) << 8) |
		       (((uint64_t)ctx->buffer[i * 8 + 7]) << 0);
	}

	for (int i = 16; i < 80; i += 1) {
		uint64_t const s0 = rrot64(w[i - 15], 1) ^
				    rrot64(w[i - 15], 8) ^ (w[i - 15] >> 7);
		uint64_t const s1 = rrot64(w[i - 2], 19) ^
				    rrot64(w[i - 2], 61) ^ (w[i - 2] >> 6);

		w[i] = w[i - 16] + s0 + w[i - 7] + s1;
	}

	for (int i = 0; i < 80; i += 1) {
		uint64_t const s1 = rrot64(e, 14) ^ rrot64(e, 18) ^
				    rrot64(e, 41);
		uint64_t const ch = (e & f) ^ (~e & g);
		uint64_t const temp1 = h + s1 + ch + K[i] + w[i];
		uint64_t const s0 = rrot64(a, 28) ^ rrot64(a, 34) ^
				    rrot64(a, 39);
		uint64_t const maj = (a & b) ^ (a & c) ^ (b & c);
		uint64_t const temp2 = s0 + maj;

		h = g;
		g = f;
		f = e;
		e = d + temp1;
		d = c;
		c = b;
		b = a;
		a = temp1 + temp2;
	}

	ctx->a += a;
	ctx->b += b;
	ctx->c += c;
	ctx->d += d;
	ctx->e += e;
	ctx->f += f;
	ctx->g += g;
	ctx->h += h;
}

static void sha512_push_byte(struct sha512_context *ctx, uint8_t byte)
{
	ctx->buffer[ctx->length[0] % SHA512_BLOCK_SIZE] = byte;
	ctx->length[0] += 1;
	if (ctx->length[0] == 0) {
		ctx->length[1] += 1;
	}
	if (ctx->length[0] % SHA512_BLOCK_SIZE == 0) {
		sha512_compress(ctx);
	}
}

void sha512_update(struct sha512_context *ctx, void const *data, size_t len)
{
	uint8_t const *buffer = data;

	for (size_t i = 0; i < len; i += 1) {
		sha512_push_byte(ctx, buffer[i]);
	}
}

static void sha512_predigest(struct sha512_context *ctx)
{
	uint8_t original_length[16];

	{
		uint64_t shifted[2];

		/* multiply by 8 with manual carry */
		shifted[1] = (ctx->length[1] << 3) | (ctx->length[0] >> 61);
		shifted[0] = ctx->length[0] << 3;

		for (int i = 0; i < 8; i += 1) {
			original_length[i] = (shifted[1] >> ((7 - i) * 8)) &
					     0xff;
		}

		for (int i = 0; i < 8; i += 1) {
			original_length[i + 8] = (shifted[0] >> ((7 - i) * 8)) &
						 0xff;
		}
	}

	sha512_push_byte(ctx, 0x80); // append bit

	// pad with zeroes
	while (ctx->length[0] % SHA512_BLOCK_SIZE != 112) {
		sha512_push_byte(ctx, 0x00);
	}

	sha512_update(ctx, original_length, 16);
}

void sha512_digest(struct sha512_context *ctx,
		   uint8_t digest[SHA512_DIGEST_SIZE])
{
	sha512_predigest(ctx);

	uint64_t const registers[8] = { ctx->a, ctx->b, ctx->c, ctx->d,
					ctx->e, ctx->f, ctx->g, ctx->h };

	for (int i = 0; i < 8; i += 1) {
		digest[i * 8 + 0] = (registers[i] >> 56) & 0xff;
		digest[i * 8 + 1] = (registers[i] >> 48) & 0xff;
		digest[i * 8 + 2] = (registers[i] >> 40) & 0xff;
		digest[i * 8 + 3] = (registers[i] >> 32) & 0xff;
		digest[i * 8 + 4] = (registers[i] >> 24) & 0xff;
		digest[i * 8 + 5] = (registers[i] >> 16) & 0xff;
		digest[i * 8 + 6] = (registers[i] >> 8) & 0xff;
		digest[i * 8 + 7] = (registers[i] >> 0) & 0xff;
	}
}

void sha384_digest(struct sha512_context *ctx,
		   uint8_t digest[SHA384_DIGEST_SIZE])
{
	sha512_predigest(ctx);

	uint64_t const registers[6] = { ctx->a, ctx->b, ctx->c,
					ctx->d, ctx->e, ctx->f };

	for (int i = 0; i < 6; i += 1) {
		digest[i * 8 + 0] = (registers[i] >> 56) & 0xff;
		digest[i * 8 + 1] = (registers[i] >> 48) & 0xff;
		digest[i * 8 + 2] = (registers[i] >> 40) & 0xff;
		digest[i * 8 + 3] = (registers[i] >> 32) & 0xff;
		digest[i * 8 + 4] = (registers[i] >> 24) & 0xff;
		digest[i * 8 + 5] = (registers[i] >> 16) & 0xff;
		digest[i * 8 + 6] = (registers[i] >> 8) & 0xff;
		digest[i * 8 + 7] = (registers[i] >> 0) & 0xff;
	}
}
